...
void task_1(void)
{
	...
	{
		if (strcmp(income_command, "L1") == 0)
		{
			PORTB |= _BV(PORTB1);
			PORTB &= ~(_BV(PORTB2) | _BV(PORTB3));
			UART_transmit_data("OK\r\n");
		} 
		else if (strcmp(income_command, "L2") == 0)
		{
			PORTB |= _BV(PORTB1) | _BV(PORTB2);
			PORTB &= ~_BV(PORTB3);
			UART_transmit_data("OK\r\n");
		}
		else if (strcmp(income_command, "L3") == 0)
		{
			PORTB |= _BV(PORTB1) | _BV(PORTB2) | _BV(PORTB3);
			UART_transmit_data("OK\r\n");
		}
		else if (strcmp(income_command, "OFF") == 0)
		{
			PORTB &= ~(_BV(PORTB1) | _BV(PORTB2) | _BV(PORTB3));
			UART_transmit_data("OK\r\n");
		}
